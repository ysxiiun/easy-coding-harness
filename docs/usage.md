...
easy-coding-harness 解决这些问题：**6 个工作阶段 + 2 个终态、用户决策边明确确认**的状态机控制 AI 行为，**任务持久化**让进度跨会话保存，**记忆系统**让过往决策和教训在新任务中被参考，**跨 Agent 交接**让你在不同 AI 平台间无缝切换。

## 技术亮点

- **三平台原生支持**：Claude Code、Codex、Qoder——安装后 Agent 原生识别 Skills，无需额外配置
- **用户决策硬门控**：用户关联边通过 `pending_transition` 等待确认；INIT → ANALYSIS、MEMORY → COMPLETE 自动流转，VERIFICATION 仍要求新鲜验证证据
...
除自动边外，阶段完成后状态仍停留在当前阶段，同时写入待确认的 `pending_transition`。你通常有三个选择：
...
**必须显式确认**才会改变用户决策边的任务状态。INIT → ANALYSIS 在初始化工作完成后自动流转；MEMORY → COMPLETE 在记忆处理完成后自动流转。其他前进、修复和重规划回退边仍保留确认门禁。
...
显式 `doc` / `analysis` / `report` 只读任务是例外：不生成 `test-strategy.md`；IMPLEMENT 必须留下匹配的 dispatch/result，由只读子代理返回完整 deliverable，主 Agent 原样展示后自动进入 COMPLETE。此类任务不进入 REVIEW、VERIFICATION 或 MEMORY，也不写任务记忆。

...
ANALYSIS 结束时生成以下紧凑方案回执，并在客户端支持时提供可点击的绝对本地文件链接；
否则提供可复制的绝对路径：
...
如果回执后还要调用原生选择或状态工具，前面的文本可能被客户端折叠为工作过程，因此不能
算作最终展示。原生选择返回或迁移调用结束后的最后一条消息会重新包含完整紧凑回执与 Dev-Spec
入口：仍待确认时同时列出完整选择，确认成功时注明接受的分支与目标阶段。Auto 不增加暂停，
Dev-Spec 入口会保留到同轮下一条最终消息。
...
- 调整测试策略（推翻不测判定、增减测试点）
- 修改文件编码声明
- 直接使用本轮改动的机械最低模式，能 Fast 就 Fast，无需选择或推荐档位

Workflow 采用本轮实际修改的机械最低模式。Fast 用于单个实际修改仓库、最多三个内聚 Unit、
...
- 优先最小且内聚的实现，不预建 wrapper/factory/层级/扩展点，也不把连贯逻辑拆成大量只调用
  一次的小方法；只有清晰语义边界、真实复用、独立测试或显著降低复杂度时才提取
...
- 新增核心 Java 类的每个方法和字段、已有核心类中新增或实质修改的方法和字段必须有
  有意义的多行 Javadoc；但接口方法已有完整、准确且可访问的 Javadoc，并且实现方法没有
  增加该接口方法文档未说明的契约、约束、副作用或其他实现特有行为时，无需重复编写，也不为
  普通门禁添加空洞的 `{@inheritDoc}`。实现存在特殊行为或项目硬规则明确要求时仍须说明差异。普通单行说明使用
  `//`。旧项目中泛化的“每个核心 Java 方法都写 Javadoc”不单独覆盖该例外，只有明确要求
  实现方法重复接口 Javadoc 的项目规则才覆盖。核心/复杂逻辑用必要行内注释说明意图、约束或
  非直观取舍，不批量补齐未触碰的历史代码
...
1. 从已有方案、实施结果和 Review 结论提炼可复用短期知识，说明适用场景、关键结论及必要原因
...
记忆以业务规则、设计原因、代码入口、适用边界和踩坑经验为正文，引用已有证据；验收 JSON、
指纹、审批与单次测试统计保留在任务日志，不复制到记忆。普通和派发协作都不为 MEMORY
增加报告、重新扫描或重复验证。没有新增可复用知识时写明原因，并设置 `memory_value: none`
和 `target_long: NONE`，不拼凑经验。升级保留旧记忆与用户模板，新 `ec-memory` 指令优先于旧
模板中的过程栏目；后续分析优先读取知识摘要，蒸馏时只归并有价值的结论。

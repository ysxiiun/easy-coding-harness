...
Agent 必须实际调用当前平台原生的选项功能展示对应业务分支，并使用原生 free-form Other 承接修改意见；仅当平台没有原生选项能力时，才按普通门或上述特殊 IMPLEMENT 门退回对应的完整纯文本编号选项。原生选择返回空值、被取消、超时或无法解析时，任务继续停留在当前阶段并保留 `pending_transition`；同一 assistant 轮最多重试一次，重试仍失败则停止当前轮并在下一次用户交互重新展示，不能无限调用或只提示“回复确认执行”。

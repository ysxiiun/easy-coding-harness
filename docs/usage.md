...
easy-coding-harness 解决这些问题：**6 个工作阶段 + 2 个终态、每条状态边默认确认**的状态机控制 AI 行为，**任务持久化**让进度跨会话保存，**记忆系统**让过往决策和教训在新任务中被参考，**跨 Agent 交接**让你在不同 AI 平台间无缝切换。

## 技术亮点

- **三平台原生支持**：Claude Code、Codex、Qoder——安装后 Agent 原生识别 Skills，无需额外配置
- **全阶段硬门控**：每个阶段边界通过 `pending_transition` 等待用户确认，VERIFICATION 要求新鲜验证证据，MEMORY 保留长期记忆阈值门禁
...
```
用户：/ec-workflow
Agent：> **Easy Coding** · Ready · ...
       没有活跃任务。请描述你的需求。

用户：给用户列表页添加搜索功能，支持按用户名和邮箱搜索
```

Agent 会创建任务并进入 INIT；INIT 完成后先等待你确认是否进入 ANALYSIS。

#### 2. 需求分析（ANALYSIS）

Agent 会读取项目代码、RULES、ABSTRACT、过往记忆，输出一份完整的技术方案，包含：
...
每个阶段完成后，状态仍停留在当前阶段，同时写入待确认的 `pending_transition`。你通常有三个选择：
...
**必须显式确认**才会改变任务状态。该门禁覆盖 INIT → ANALYSIS、ANALYSIS → IMPLEMENT、IMPLEMENT → REVIEW、REVIEW → VERIFICATION、VERIFICATION → MEMORY、MEMORY → COMPLETE，也覆盖修复和重规划回退边。

Agent 必须优先使用当前平台原生的选项功能展示这三个分支（包括原生 free-form Other）；仅当平台没有原生选项能力时，才退回纯文本编号选项。
...
Agent 进行多维度审查（正确性、规范性、完整性、测试、安全），给出分级判定。需要进入 VERIFICATION、IMPLEMENT 或 ANALYSIS 时，先展示标准确认门禁。

#### 6. 验证（VERIFICATION）

lint + typecheck + test 三项并行执行。全部通过后展示结果，等待你手动验收。
...
handoff 会保留当前阶段和 `pending_transition`。因此 IMPLEMENT 完成后可交给另一个 Agent 做 REVIEW，REVIEW 完成后也可交给另一个做 VERIFICATION；接手方不会重复执行已完成阶段。
...
- **原位迁移**：config.yaml 仅更新 `harness_version`；0.5.x 的 `task.json` 和 session 仅迁移旧状态名、阶段历史及恢复进度元数据

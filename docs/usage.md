...
easy-coding-harness 解决这些问题：**8 个阶段 + 2 个终态的硬门控状态机**控制 AI 行为，**任务持久化**让进度跨会话保存，**记忆系统**让过往决策和教训在新任务中被参考，**跨 Agent 交接**让你在不同 AI 平台间无缝切换。

## 技术亮点

- **三平台原生支持**：Claude Code、Codex、Qoder——安装后 Agent 原生识别 Skills，无需额外配置
- **全阶段硬门控**：WAITING_CONFIRM 阻塞用户确认、VERIFICATION 要求新鲜验证证据、归档需用户验收
...
Agent 自动进行多维度审查（正确性、规范性、完整性、测试、安全），给出分级判定。如果发现问题，会回退到 IMPLEMENT 修复。
...
不限于方案确认阶段——IMPLEMENT 完成后交给另一个 Agent 做 REVIEW，REVIEW 完成后交给另一个做 VERIFICATION，都是合法的。
...
- **不碰**：config.yaml（仅更新 harness_version）、tasks/、memory/、SOUL.md、RULES.md、ABSTRACT.md 等用户资产

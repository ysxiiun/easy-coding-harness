...
`codex-019f....json`；agent 前缀避免 Claude Code、Codex、Qoder 的 ID 冲突。若平台未提供
逻辑 session ID，才降级为 `<agent>-ppid-<ppid>.json`。session 同时记录原始 ID、来源、
创建时间和最后活跃时间；无当前任务的长期空闲 session 才会被清理。

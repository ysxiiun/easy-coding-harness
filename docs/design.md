...
**交接机制**：
- `state.json` 记录 `last_agent` 字段，接手 Agent 能区分"交接过来的任务"和"自己上次中断的任务"
...
用户确认设计后，自动创建任务目录和 task.json，更新 state.json，进入 ANALYSIS 阶段——无需用户手动调用 ec-workflow。
...
幂等设计——检查 state.json 是否存在且合法，已就绪则立即退出。多次调用与单次调用效果一致。
...
- state.json 记录 `last_agent` 字段，接手 Agent 能识别"这是交接过来的任务"
...
- **提交范围保护**：`.easy-coding/` 变更默认纳入提交（state.json 始终排除）
- **进行中任务提醒**：提交涉及未完成任务时警告
- **冲突处理**：`.easy-coding/` 内冲突必须先说明明细，确认后做归纳式合并
- **跨仓库提交**：读取 state.json 的 repo_paths，逐个仓库检查和提交

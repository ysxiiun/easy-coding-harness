...
知识摘要与正文，跳过无新增知识的记录。旧报告只读取命中的可复用结论，追溯任务时才读取
验收与过程证据；不默认加载最近若干条记忆或完整记忆库。Fast 候选只补充最近邻同类代码、直接合同和
...
- 正文回答后续开发需要知道什么：业务语义、设计原因、修改入口、适用边界、真实故障原因与修复经验；短期记忆在蒸馏前即可直接使用
- 标题与摘要面向知识检索；内容按实际需要组织，不强制填满分类。确实没有新增知识时，使用 `memory_value: none` / `target_long: NONE` 并简短说明
- 验收记录、审批、指纹、单次测试统计与交接流水保留在 `execution.jsonl`；记忆引用来源，不复制这些字段，也不罗列“不沉淀内容”
- 主 Agent 复用已确认方案、实施结果与 Review 发现完成提炼；普通和派发协作规则一致，不增加报告、扫描或验证轮次
- 升级保留用户模板和已有记忆；新 `ec-memory` 指令优先于旧模板的过程栏目，不新增 schema、迁移或知识评分门禁
- 窗口外的旧记忆成为长期沉淀候选
...
**沉淀流程**：
1. 按 domain/tags/related_files 渐进式加载匹配知识，在冻结候选内提炼事实，过滤旧报告的过程噪声与无知识记录（不做无边界全仓扫描）

...
- **约束严格度不降级**：固定状态机、读写分离、全阶段确认门控全部保留。拆分为独立 Skill 后约束力度不变，只是组织方式从单文件变成了结构化的 Skills 体系。
- **项目模式检测保留**：自动区分初创/迭代项目并调整行为策略，这是 easy-coding 独有的能力。

从 Skill 升级到 Harness，用户的项目知识文件、记忆数据、编码规范零迁移成本。

### 三、六工作阶段 + 全边确认严格约束编码流程

三个对标项目（easy-coding、Trellis、superpowers）中**最严格的**控制系统。

**状态机**：INIT → ANALYSIS → IMPLEMENT → REVIEW → VERIFICATION → MEMORY → COMPLETE，不允许跳阶段。"简单任务"有简短的分析，没有被跳过的分析。

**全边确认 + 专项硬门控**：

1. **全阶段迁移确认**——阶段完成后只写 `pending_transition`，默认优先通过智能体原生选项功能让用户选择确认目标阶段、交接给其他智能体或 free-form Other；未经确认不改变 `status`，纯文本编号仅作能力回退。
2. **VERIFICATION 门控**——lint + typecheck + test 必须全部通过，必须是本轮新鲜执行的结果。"should pass" 不是证据，上一轮的结果不算数。
...
- 每个阶段边界都提供显式交接入口，handoff 保留当前 `pending_transition`
...
- **pending_transition**：每个阶段完成后记录待确认边；用户显式确认前不迁移。标准分支为确认目标阶段、交接给其他智能体和 free-form Other，优先由平台原生选项功能承载。
- **VERIFICATION**：lint + typecheck + test 必须全部通过，且必须是本轮新鲜执行的结果——上一轮的结果不算，"should pass" 不是证据。
- **MEMORY**：归档前必须用户验收；进入后先写短期记忆，再执行长期记忆阈值门禁，完成后仍需确认才能进入 COMPLETE。
...
7. 用户确认；任务创建进入 INIT 后，仍按全阶段门禁确认是否进入 ANALYSIS
...
用户确认设计后，通过状态 API 自动创建任务目录和 task.json、更新当前任务指针并进入 INIT；INIT 完成后展示标准迁移门禁，再由用户确认是否进入 ANALYSIS。

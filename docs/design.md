...
- **约束严格度不降级**：固定状态机、读写分离、用户决策确认门控和 VERIFICATION 硬门全部保留。拆分为独立 Skill 后约束力度不变，只是组织方式从单文件变成了结构化的 Skills 体系。
- **项目模式检测保留**：自动区分初创/迭代项目并调整行为策略，这是 easy-coding 独有的能力。

从 Skill 升级到 Harness，用户的项目知识文件、记忆数据、编码规范零迁移成本。

### 三、六工作阶段 + 用户决策确认严格约束编码流程
...
1. **用户决策边确认**——INIT → ANALYSIS、MEMORY → COMPLETE 以及受校验的只读 IMPLEMENT → COMPLETE 使用自动边；其他阶段完成后写 `pending_transition`，由用户确认或交接。代码 IMPLEMENT 完成后还可选择跳过 REVIEW 进入 VERIFICATION。
...
**修复循环有范围守卫**：用户验收阶段的修改请求会对照 dev-spec 判断范围——范围内的修复回到 IMPLEMENT，完成后由用户重新选择进入 REVIEW 或直接进入 VERIFICATION；范围外的建议创建新任务。不允许静默吸收 scope creep。
...
- 每个用户决策边界都提供显式交接入口，handoff 保留当前 `pending_transition`；INIT → ANALYSIS 与 MEMORY → COMPLETE 为自动边
...
- **自动边**：INIT 工作完成后进入 ANALYSIS；MEMORY 检查点完成后进入 COMPLETE；只读任务在 IMPLEMENT 展示并校验完整报告后直接进入 COMPLETE。自动边不生成 `pending_transition`，不提供交接。
- **pending_transition**：其余用户决策边记录待确认目标；用户显式确认前不迁移。代码 IMPLEMENT 完成后额外提供“进入 REVIEW / 跳过 REVIEW 进入 VERIFICATION / 交接”分支。
- **VERIFICATION**：lint + typecheck + test 必须全部通过，且必须是本轮新鲜执行的结果——上一轮的结果不算，"should pass" 不是证据。
- **MEMORY**：归档前必须用户验收；进入后先写短期记忆，再执行长期记忆阈值门禁，完成后自动进入 COMPLETE。

...
- 任务交接成功后统一返回简短、可复制的 `ec-workflow` 提示词，包含真实项目绝对路径、任务 ID
  和正确的认领或审查验证动作；失败或取消时不展示成功提示。
- 普通交接、实施派发和 QUALITY 修复往返共享同一提示词约定；范围、授权、进度与验证证据仍保存在
  原任务和交接记录中，必要提醒最多摘取一条，避免依赖提示词传递重要信息。

## 1.1.0

- 将 `1.1.0-beta.4` 晋升为正式版本，包含局部修复与验证证据复用、UT 单测策略、人工派发协作、
  可复用知识记忆，以及 Hook 状态读取、验收快照和 Canonical 写回的效率优化。
- 默认 npm `latest` 安装获取 `1.1.0`；预发布用户可直接升级到本版本。
...
- 新增 `cooperate_mode: default | dispatch`，支持用户手动派发实现和 QUALITY 内修复，主 Agent
  继续负责分析、验证与沉淀；修复范围和执行者一次确认，接手后沿用授权。
- 普通修复在 QUALITY 内闭环，只重新验证受影响输入；需求、契约或主体方案变化才返回分析或实施。
- 行为配置统一按会话 > 本地 > 项目解析；`~/.easy-coding/config.yaml` 仅在用户保存本地设置时创建，
  支持逐字段覆盖与恢复继承，保留进行中任务的单测合同和交接进度。
- 交接返回下一步、范围、证据引用和停止位置；按会话复用 Spec 消费记录，避免往返覆盖和重复确认。
- 检查支持批量准备与登记，命令参数等价时复用结果；同次操作共享日志、计划与状态快照，历史指纹不迁移。

## 1.1.0-beta.1

- 新增与 TDD 并列的 UT 单测覆盖策略，统一使用 `unit_test_mode: none | ut | tdd`。
  UT 只要求通过的本地单测与达到阈值的改动行覆盖率，不要求 RED/GREEN 过程或独立 TDD 审查。
- 覆盖率阈值统一为 `ut_coverage_threshold`，UT/TDD 共用，默认 90；一次测试同时生成测试与
  覆盖率证据，相同输入继续复用结果。
- 配置 schema 升至 6，迁移旧 TDD 开关和阈值，保留项目/session 继承关系、任务冻结配置、
  基线与执行进度。三平台配置入口、状态展示和使用说明同步支持三种策略。
- 升级保留活动 UT/TDD 任务使用的覆盖率工具，避免工具替换导致已有证据整体失效；
  任务结束后再次运行 `easy-coding upgrade` 即可更新工具，无需重新安装或清空任务。
